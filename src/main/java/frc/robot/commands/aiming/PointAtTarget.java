...
public class PointAtTarget extends Command {
  ...
  public void calcAngle(){
    ...
    if(vision.getNormToSpeaker() > 3.85){
      desiredAng += vision.getNormToSpeaker() * 0.6;
  ...
  public void execute(){
      ...
      else if(Math.abs(desiredAng-arm.getPositionDeg()) < 0.4){
        CommandScheduler.getInstance().cancel(this);
      }
      else{
  ...
  public boolean isFinished() {
    ...
    if(Math.abs(arm.getPositionDeg()-desiredAng) < 0.4){
      return true;
    }
